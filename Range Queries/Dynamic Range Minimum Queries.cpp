...
void update_tree(ll ind,ll val){
    seg_tree[n+ind]=val;
    for(ll i=n+ind;i>1;i>>=1)seg_tree[i>>1]=min(seg_tree[i],seg_tree[i^1]);
}
ll seg_query(ll l, ll r){
    ...
    for (; l < r; l >>= 1, r >>= 1){
        if (l&1)
            res = min(seg_tree[l++],res);

        if (r&1)
            res = min(seg_tree[--r],res);
    }
    return res;
}
void solve(){
    ...
    while(q--){
        ...
        ll a,b;cin>>a>>b;
        if(test==1){
            update_tree(a-1,b);continue;
        }
        cout<<seg_query(a-1,b)<<endl;
    }
}
int main(){
        KIRA;
