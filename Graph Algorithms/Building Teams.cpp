...
void dfs(ll node,ll parent){
    if(biparte[node]==-1){
        biparte[node]=!biparte[parent];
    }
    else{
        if(biparte[node]==biparte[parent]&&node!=parent){wrong=1;return;}
    }
    for(auto x:g[node]){
        if(biparte[x]==-1)dfs(x,node);
        if(biparte[x]==biparte[node]){wrong=1;return;}
    }
...
void solve(){
    ...
    rep(i,1,n+1){
        ...
        if(wrong){cout<<"IMPOSSIBLE"<<endl;return;}
    }
    rep(i,1,n+1)cout<<biparte[i]+1<<" ";
}
int main(){
        KIRA;
        //READ;
